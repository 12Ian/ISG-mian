...
def run(payload: dict, context) -> dict:
    ...
    labeled_samples = [sample for sample in samples if (sample.get("labels") or sample.get("labels_json") or [])]
...
def _run_texture_augmentation(payload, context, output_dir, samples, target_count, method):
    ...
    for index in range(target_count):
        ...
        context.set_progress((index + 1) * 100 / target_count, f"{method} classical {index + 1}/{target_count}")
    return {"ok": True, "outputs": outputs, "logs": ["used texture augmentation path"]}
