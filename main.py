...
class BackendService(QObject):
        ...
        def worker():
            try:
                ...
                if result.get("ok"):
                    ...
                    self.generationStatusUpdated.emit(result.get("message", "生成任务执行失败"), False, 0.0)
            except Exception as exc:
    ...
    def createCleaningTask(self, datasetId: int, modality: str, parameters: dict) -> dict:
        ...
        if not result.get("ok"):
            self.cleaningStatusUpdated.emit(result.get("message", "清洗任务创建失败"), False)
            return {"status": "error", "message": result.get("message", "未知错误")}
    ...
    def _run_training_in_background(self, task_id: int):
        def worker():
            try:
                ...
                if result.get("ok"):
                    self.trainingStatusUpdated.emit("训练任务已完成", True, 100.0)
                else:
                    self.trainingStatusUpdated.emit(result.get("message", "训练任务执行失败"), False, 0.0)
            except Exception as exc:
