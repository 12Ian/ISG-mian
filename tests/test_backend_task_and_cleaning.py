...
def test_text_deduplicate_plugin_writes_cleaned_text_and_separates_modes(tmp_path):
    from plugins.cleaning import text_deduplicate

    source_file = tmp_path / "dups.txt"
    source_file.write_text("alpha\nalpha\nBeta\nbeta\n我我我哦我我", encoding="utf-8")
    ...
    assert suggestion["details"]["duplicate_count"] == 3
    cleaned_path = Path(suggestion["details"]["output_file_path"])
    assert cleaned_path.read_text(encoding="utf-8") == "alpha\nalpha\nBeta\nbeta\n我哦我"

    cleaned_lines, duplicate_lines = text_deduplicate._deduplicate("一句话，一句话，另一句\n同一行\n同一行", "line")
    assert cleaned_lines == "一句话，另一句\n同一行"
    assert duplicate_lines == 2


def test_seeded_text_deduplicate_algorithm_runs_through_backend(tmp_path):
    ...
    monitor_item = suggestions["monitor_items"][0]
    assert monitor_item["preview_path"] == str(output_path)
    assert monitor_item["preview_sample_id"] == monitor_item["sample_id"]
