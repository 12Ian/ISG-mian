...
def test_diffusion_image_augmenter_handles_time_embedding_shape(tmp_path):
    torch = pytest.importorskip("torch")
    from plugins.generation.diffusion_image_augmenter import _build_model
...
def test_environment_simulation_plugin_reads_frontend_parameter_names(tmp_path):
    from plugins.generation.environment_simulation_image_augmenter import run
    ...
    result = run(
        {
            "algorithm_key": "generation.image.environment_simulation",
            "parameters": {
                "fog_intensity": 0.8,
                "snow_intensity": 0.6,
            ...
            "target_count": 1,
            "input": {"samples": [{"id": 1, "sample_path": str(source), "path": str(source), "name": "source.jpg"}]},
...
def test_style_controlled_plugin_reads_frontend_parameter_names(tmp_path):
    from plugins.generation.style_controlled_text_augmenter import run

    source = tmp_path / "source.txt"
    source.write_text("其实这个方案非常不错，并且可以同时兼顾效率和质量。", encoding="utf-8")
    ...
    result = run(
        {
            "algorithm_key": "generation.text.style_controlled",
            "parameters": {
                "style_label": "concise",
                "conciseness_strength": 1.0,
            },
            "target_count": 1,
            "input": {"samples": [{"id": 1, "sample_path": str(source), "path": str(source), "name": "source.txt"}]},
            "output": {"output_dir": str(tmp_path / "generated")},
        },
        Context(),
    )
...
def test_style_controlled_plugin_changes_text_for_concise_style(tmp_path):
    from plugins.generation.style_controlled_text_augmenter import run

    source = tmp_path / "source.txt"
    original = "其实这个方案非常不错，并且可以同时兼顾效率和质量。"
    ...
    result = run(
        {
            ...
            "target_count": 1,
            "input": {"samples": [{"id": 1, "sample_path": str(source), "path": str(source), "name": "source.txt"}]},
            "output": {"output_dir": str(tmp_path / "generated2")},
        },
        Context(),
    )

    out = Path(result["outputs"][0]["output_path"]).read_text(encoding="utf-8")
    assert result["ok"] is True
    assert out != original


def test_context_embedding_plugin_reads_frontend_parameter_names(tmp_path):
    from plugins.generation.context_embedding_text_augmenter import run

    source = tmp_path / "source.txt"
    original = "好消息带来了新的机会，同时也意味着更高的要求。"
    source.write_text(original, encoding="utf-8")
    ...
    result = run(
        {
            "algorithm_key": "generation.text.context_embedding",
            "parameters": {
                "mask_ratio": 1.0,
                "cross_lingual_strength": 1.0,
            },
            "target_count": 1,
            "input": {"samples": [{"id": 1, "sample_path": str(source), "path": str(source), "name": "source.txt"}]},
            "output": {"output_dir": str(tmp_path / "generated3")},
        },
        Context(),
    )
...
def test_text_noise_phrase_backtranslation_and_synonym_plugins_modify_text(tmp_path):
    from plugins.generation.structure_noise_text_augmenter import run as structure_run
    from plugins.generation.vocabulary_phrase_text_augmenter import run as vocab_run
    from plugins.generation.back_translation_text_augmenter import run as back_run
    from plugins.generation.synonym_replacement_text_augmenter import run as synonym_run

    source = tmp_path / "source.txt"
    original = "这个方案非常好，因为它可以同时提升效率和质量。"
    source.write_text(original, encoding="utf-8")
    ...
    common_input = {"samples": [{"id": 1, "sample_path": str(source), "path": str(source), "name": "source.txt"}]}

    results = [
        structure_run(
            {
                "algorithm_key": "generation.text.structure_noise",
                "parameters": {"perturbation_strength": 1.0},
                "target_count": 1,
                "input": common_input,
                "output": {"output_dir": str(tmp_path / "structure")},
            },
            Context(),
        ),
        vocab_run(
            {
                "algorithm_key": "generation.text.vocabulary_phrase",
                "parameters": {"replacement_ratio": 1.0, "phrase_level": True},
                "target_count": 1,
                "input": common_input,
                "output": {"output_dir": str(tmp_path / "vocab")},
            },
            Context(),
        ),
        back_run(
            {
                "algorithm_key": "generation.text.back_translation",
                "parameters": {"intermediate_language": "en", "back_translate_probability": 1.0, "sentence_restructure_strength": 1.0},
                "target_count": 1,
                "input": common_input,
                "output": {"output_dir": str(tmp_path / "back")},
            },
            Context(),
        ),
        synonym_run(
            {
                "algorithm_key": "generation.text.synonym_replacement",
                "parameters": {"replacement_ratio": 1.0},
                "target_count": 1,
                "input": common_input,
                "output": {"output_dir": str(tmp_path / "syn")},
            },
            Context(),
        ),
    ]
    ...
    for result in results:
        assert result["ok"] is True
        out = Path(result["outputs"][0]["output_path"]).read_text(encoding="utf-8", errors="ignore")
        assert out != original
